...
CREATE TABLE `user_present_all_received_history` (
  ...
  `deleted_at` bigint default NULL,
  PRIMARY KEY (`id`),
  KEY `idx_user_present_all_received_history_user_id_present_all_id` (`user_id`, `present_all_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
CREATE TABLE `user_one_time_tokens` (
  ...
  `expired_at` bigint NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE uniq_token (`user_id`, `token`),
  KEY `idx_user_one_time_tokens_token_token_type` (`token`, `token_type`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
